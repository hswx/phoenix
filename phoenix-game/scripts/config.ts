...
import { EmitHtmlReCompilePlugin } from './EmitHtmlReCompilePlugin';
import { ResPlugin } from './resplugin';
import { ExtCompilerPlugin } from './ExtCompilerPlugin';
    ...
    buildConfig: (params) => {
        ...
        if (command == 'build') {
            ...
            return {
                ...
                commands: [
                    new ResPlugin(),
                    new ExtCompilerPlugin(),
                    new UglifyPlugin([{
                        sources: ["main.js"],
                        target: "main.min.js"
                    }]),
                    new ManifestPlugin({ verbose: true, output: "manifest.json" }),
                ]
            }
        }
        else {
